...
pintos_raw_link="https://people.cs.uchicago.edu/~daniar/doc/$tar_name"
...
tar -zxvf ~/cs230-grading/$tar_name -C ~/cs230-grading/

rm ~/cs230-grading/pintos-raw.tgz
mv ~/cs230-grading/pintos-raw ~/cs230-grading/pintos

# avoid running the no-vm test
sed -i 's/tests\/userprog\/no-vm/ /g' ~/cs230-grading/pintos/src/userprog/Make.vars
rm -rf ~/cs230-grading/pintos/src/tests/userprog/no-vm/  &> /dev/null
...
# This function intentionally ignore the test proportions define in
# src/tests/userprog/Grading. Instead it only focuses on the weight of each test.
indexChecker() {
sum=0
...
if [ -z "$pass" ]
  then
    # Multi-oom is not counted (regarded as a bonus)
    if [ "$i" -eq 62 ]
    then
      indexChecker $i
    fi
...
grade=$(calc $sum2/100);
grade100=$(calc $(calc $grade*100)/61.3);
...
echo "Total points are: $sum2 (out of 6130)";
